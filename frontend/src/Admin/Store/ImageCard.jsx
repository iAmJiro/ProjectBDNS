...
export default function ImageCard({
  ...
  const handleDelete = () => {
    fetch(
      `${import.meta.env.VITE_APP_BACKEND_URL}/api/image/delete/${imageId}`,
      {
        method: "delete",
        headers: {
    ...
    <div className="max-w-sm bg-white dark:bg-gray-800 border border-gray-300 rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="p-4">
        {isEditing ? (
          ...
          <>
            <h5 className="text-2xl font-bold text-gray-900 dark:text-white mb-2 text-center">
              {name}
            </h5>
            <p className="text-gray-700 text-sm mb-3 dark:text-gray-300 text-center">
              {description}
            </p>
